...
async function syncStudent(request,env){
  ...
  `).bind(email,raw,summary.onenote,summary.digipen,summary.scan,summary.total,now,now,now).run();
...
async function listStudents(request,env){
  ...
  let statement='SELECT email,class_name,progress_json,onenote_done,digipen_done,scan_done,total_done,first_seen,last_seen,updated_at FROM students';
  let query;
  if(CLASSES.has(filter))query=env.DB.prepare(statement+' WHERE class_name = ? ORDER BY last_seen DESC').bind(filter);
  else if(filter==='UNASSIGNED')query=env.DB.prepare(statement+' WHERE class_name IS NULL ORDER BY last_seen DESC');
  else query=env.DB.prepare(statement+' ORDER BY COALESCE(class_name,\'ZZZ\'), last_seen DESC');
  const result=await query.all();
  const students=(result.results||[]).map(row=>{
    let progress={};try{progress=JSON.parse(row.progress_json||'{}')}catch{}
    return {...row,name:friendlyName(row.email),progress,total_possible:21};
  });
  return json({ok:true,classes:[...CLASSES],students});
}
