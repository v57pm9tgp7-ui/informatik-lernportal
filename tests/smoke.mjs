...
check('Hauptnavigation vollständig und ohne alten Grundlagenbereich',()=>{for(const id of ['start','wochen','training','fortschritt'])assert.match(index,new RegExp(`data-nav="${id}"`));assert.doesNotMatch(index,/data-nav="grundlagen"/)});
...
check('Woche 38 besitzt eine eigene frei aufrufbare Ansicht',()=>{assert.match(index,/data-view="woche-38"/);assert.match(index,/id="week38-title">Digital arbeiten/);assert.match(portalJs,/views=\['start','wochen','woche-37','woche-38','training','fortschritt'\]/)});
